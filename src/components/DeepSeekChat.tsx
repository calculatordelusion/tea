...
const DeepSeekChat = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hallo! Ich bin ein KI-Chatbot, der von DeepSeek V3 betrieben wird.",
      timestamp: new Date(),
    },
  ]);
  ...
  const callDeepSeekAPI = async (messages: any[], selectedModel: string): Promise<string> => {
    ...
    const systemPrompt = selectedModel === "deepseek-v3" 
      ? "Du bist ein KI-Chatbot, der von DeepSeek V3 betrieben wird. Du antwortest auf Deutsch und bist hilfsbereit, präzise und freundlich."
      : "Du bist ein KI-Chatbot, der von DeepSeek R1 betrieben wird. Du antwortest auf Deutsch mit besonderer Betonung auf logisches Denken und Problemlösung.";
  ...
  const handleModelChange = (newModel: string) => {
    setModel(newModel);
    const modelName = newModel === "deepseek-v3" ? "DeepSeek V3" : "DeepSeek R1";
    const welcomeText = newModel === "deepseek-v3" 
  ...
  const ModelTab = ({ value, children }: { value: string; children: React.ReactNode }) => {
      ...
      <div className="max-w-4xl mx-auto p-4">
        <div className="deepseek-chat-bg h-[600px] flex flex-col">
          ...
          <div className="flex items-center justify-center gap-4 p-4 border-b">
            <ModelTab value="deepseek-v3">DeepSeek V3</ModelTab>
            <ModelTab value="deepseek-r1">DeepSeek R1</ModelTab>
          </div>
